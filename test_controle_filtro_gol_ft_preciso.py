...
from filtro_gol_ft_antecipado_preciso import METODO, ROLLBACK
from telegram_alertas import motivo_suspensao_simulacao


class ControleFiltroGolFtPrecisoTest(unittest.TestCase):
    def setUp(self):
        self.caminho = Path.cwd() / (
            f".teste_controle_filtro_ft_{uuid4().hex}.json"
        )
        self.agora = datetime(2026, 9, 9, 21, 0, 0)
    ...
    def test_checkpoint_negativo_suspende_somente_essa_entrega(self):
        estado = aplicar_validacao({
            "versao": "validacao-v1",
            "registrado_em": "2026-09-09T21:00:00",
    ...
    def test_gateway_aplica_filtro_e_circuit_breaker_exatos(self):
        with patch.dict("os.environ", {ROLLBACK: "1"}, clear=False):
            self.assertIn(
                ...
                motivo_suspensao_simulacao(self._candidato(0)),
            )
            with patch(
                "telegram_alertas.filtro_gol_ft_preciso_liberado",
                return_value=False,
            ):
                self.assertEqual(
                    "circuit_breaker_filtro_gol_ft_antecipado_preciso",
