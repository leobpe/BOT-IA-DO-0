"""Coorte prospectiva imutável do filtro HT antecipado preciso V2.

A população usa os primeiros 60 sinais elegíveis após a âncora, tenham sido
enviados ou não. A inclusão é congelada antes de qualquer leitura do resultado;
assim pausa do Telegram e resultados posteriores não escolhem a amostra.
"""
...
VERSAO_VALIDACAO = "validacao-gol-ht-antecipado-preciso-v2"
CHAVE_DEFINICAO = f"exploracao_sombra_definicao:{VERSAO_VALIDACAO}"
TABELA_COORTE = "coorte_gol_ht_antecipado_preciso"
TAMANHO_COORTE = 60
DESENVOLVIMENTO = 42
HOLDOUT = 18
...
def registrar_ou_validar_definicao(conexao, registrado_em=None):
    ...
    if linha is not None:
        ...
        if divergencias:
            raise RuntimeError(
                "Ancora do filtro HT antecipado preciso inconsistente: "
                + ",".join(divergencias)
            )
        return existente
...
def _decisoes_pos_ancora(conexao, documento):
    """Lê somente decisões causais do gerador, sem resultados ou entregas."""
    return _linhas(
        conexao,
        """
        ...
        WHERE datetime(criado_em)>=datetime(?)
          AND mercado='gol_ht' AND status='simulacao'
...
def _funil_candidatos_pos_ancora(conexao, documento):
    """Explica a coleta prospectiva sem alterar seleção ou consultar placar."""
    ...
    for linha in linhas:
        ...
        if candidato is None:
            ...
            continue
        decisao = avaliar_filtro_gol_ht_antecipado(
    ...
    return {
        "versao": "funil-gol-ht-antecipado-preciso-v1",
        "estado": estado_funil,
...
def sincronizar_coorte(conexao, agora=None):
    """Congela elegíveis em ordem causal sem consultar resultados/entregas."""
...
def _carregar_coorte(conexao, documento):
    ...
    for ordem, linha in enumerate(linhas, 1):
        if int(linha.get("ordem") or 0) != ordem:
            raise RuntimeError("ordem_coorte_gol_ht_preciso_inconsistente")
        try:
            candidato = json.loads(linha.get("candidato_json") or "{}")
        except (TypeError, json.JSONDecodeError):
            raise RuntimeError("json_coorte_gol_ht_preciso_invalido")
        decisao = avaliar_filtro_gol_ht_antecipado(
            candidato, {ROLLBACK: "1"}
        )
        partida_id = int(linha["partida_id"])
        if (
            ...
            raise RuntimeError("membro_coorte_gol_ht_preciso_invalido")
        partidas.add(partida_id)
        itens.append({
...
def _historico_gerador(conexao, documento):
    linhas = _linhas(conexao, """
        WITH base AS (
          ...
          WHERE datetime(s.criado_em)<datetime(?)
            AND s.mercado='gol_ht' AND s.status='simulacao'
    ...
    for linha in linhas:
        ...
        base.append(item)
        if avaliar_filtro_gol_ht_antecipado(
            candidato, {ROLLBACK: "1"}
        ).get("aprovada"):
            filtrados.append((len(base) - 1, item))
    corte_base = int(len(base) * 0.70)
    desenvolvimento = [item for posicao, item in filtrados if posicao < corte_base]
    holdout = [item for posicao, item in filtrados if posicao >= corte_base]
    return {
...
def resumir_validacao(conexao):
    ...
    return {
        ...
        **total,
        ...
        "politica_seguranca": {
            "checkpoint_fixo": CHECKPOINT_SEGURANCA,
            "efeito": "suspender_somente_entrega_ht_antecipado_preciso",
