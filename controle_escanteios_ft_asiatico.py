"""Circuit breaker isolado do escanteio asiatico FT prospectivo."""
...
VERSAO_CONTROLE = "controle-escanteios-ft-asiatico-prospectivo-v1"
ARQUIVO_ESTADO = Path(__file__).with_name(
    "escanteios_ft_asiatico_estado.json"
)
...
def suspender(
    ...
    documento = {
        ...
        "reativacao": (
            "python controlar_escanteios_ft_asiatico.py --ativar"
        ),
    }
    return {
...
def ativar(caminho=None, agora=None, motivo="reativacao_manual"):
    ...
    documento = {
        ...
        "metricas": {},
        "rollback": (
            "python controlar_escanteios_ft_asiatico.py --suspender"
        ),
    }
    return {
        **_gravar(caminho, documento),
...
def aplicar_validacao(validacao, *, caminho=None, agora=None):
    ...
    if (inconsistente or negativo or concluida) and estado.get("ativo"):
        ...
        motivo = (
            "validacao_escanteios_ft_asiatico_inconsistente"
            if inconsistente else
            "ic95_roi_checkpoint_25_integralmente_negativo"
            if negativo else
            "coorte_100_concluida_aguardando_revisao_manual"
        )
        estado = suspender(
            assinatura,
            motivo,
            metricas={
                ...
                "half_greens": int(
                    validacao.get("half_greens", 0) or 0
                ),
                "voids": int(validacao.get("voids", 0) or 0),
                "half_reds": int(validacao.get("half_reds", 0) or 0),
