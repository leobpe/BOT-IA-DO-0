"""Coorte prospectiva fixa da política principal protegida de Gol HT."""
...
VERSAO_VALIDACAO = "validacao-gol-ht-principal-protegido-v1"
CHAVE = f"exploracao_sombra_definicao:{VERSAO_VALIDACAO}"
TAMANHO_COORTE = 100
DESENVOLVIMENTO = 70
HOLDOUT = 30
MINIMO_VALIDOS_TOTAL = 95
MINIMO_VALIDOS_DESENVOLVIMENTO = 67
...
def definicao():
    nucleo = {
        ...
        "holdout": HOLDOUT,
        "criterio": (
            "roi_ic95_inferior_positivo_e_green_ic95_acima_break_even"
        ),
        "evidencia_geradora": {
            "amostra_modelavel": 143,
            "greens": 79,
            "reds": 64,
            "roi": -0.0187,
            "perfis_estaveis_tres_fatias": 0,
        },
        "telegram": False,
        "promocao_automatica": False,
        "reativacao_somente_manual": "GOL_HT_PRINCIPAL_OFICIAL_ATIVO=1",
    }
    canonico = json.dumps(
...
def registrar_ou_validar_gol_ht_protegido(conexao, registrado_em=None):
    esperada = definicao()
    ...
    if linha is not None:
        ...
        if divergencias:
            raise RuntimeError(
                "Validação Gol HT protegido diverge da âncora imutável: "
                + ",".join(divergencias)
            )
        return existente
    documento = {
...
def auditar_validacao_gol_ht_protegido(conexao, exigir_registro=False):
    linha = conexao.execute(
        "SELECT valor FROM metadados WHERE chave=?", (CHAVE,)
    ).fetchone()
    if linha is None:
...
def _metricas(linhas):
    validas = [
        ...
        and item["odd"] is not None
        and float(item["odd"]) > 1.0
    ]
    retornos = [float(item["retorno_unidades"]) for item in validas]
    greens = sum(
        item["resultado"] in {"green", "half_green"} for item in validas
    )
    taxa = greens / len(validas) if validas else None
    break_even = (
        statistics.fmean(1.0 / float(item["odd"]) for item in validas)
        if validas else None
    )
    intervalo_green = intervalo_wilson(greens, len(validas))
    intervalo_roi = None
    if len(retornos) >= 2:
...
def resumir_validacao_gol_ht_protegido(conexao):
    linha = conexao.execute(
        "SELECT valor FROM metadados WHERE chave=?", (CHAVE,)
    ).fetchone()
    if linha is None:
    ...
    documento = json.loads(linha["valor"])
    auditoria = auditar_validacao_gol_ht_protegido(conexao, True)
    ...
    linhas = conexao.execute(
        ...
        WITH independentes AS (
          SELECT s.id,s.partida_id,s.criado_em,s.regra_fingerprint,s.odd,
          ...
          WHERE datetime(s.criado_em)>=datetime(?)
            ...
            AND s.regra_versao=?
            AND json_extract(
                  s.features_json,'$.gol_ht_protegido.modo'
                )='validacao_sombra'
        )
    ...
    ic_roi = total["intervalo_roi_95"]
    ic_green = total["ic95_taxa_green"]
    if not coorte_fechada:
        ...
        and ic_roi is not None
        and ic_roi[0] > 0
        and ic_green is not None
        and total["break_even_medio"] is not None
        and ic_green[0] > total["break_even_medio"]
