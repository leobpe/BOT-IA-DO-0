"""Circuit breaker persistente e isolado do filtro pre-live preciso.

O controle atua somente na entrega. A geracao, a persistencia e a liquidacao
dos bilhetes continuam formando a coorte prospectiva mesmo quando o envio e
suspenso. Estado ausente libera; estado corrompido ou incompativel falha
fechado.
"""
...
VERSAO_CONTROLE = "controle-pre-live-preciso-v1"
ARQUIVO_ESTADO = Path(__file__).with_name("pre_live_preciso_estado.json")
...
def suspender(
    ...
    *,
    ...
    documento = {
        ...
        "ativo": False,
        "estado": (
            "suspenso_automatico" if automatico else "suspenso_manual"
        ),
        "motivo": str(motivo),
        "assinatura": assinatura,
        "atualizado_em": instante,
        "metricas": dict(metricas or {}),
        "reativacao": "python controlar_pre_live_preciso.py --ativar",
    }
    return {
...
def ativar(caminho=None, agora=None, motivo="reativacao_manual"):
    ...
    documento = {
        ...
        "atualizado_em": instante,
        "metricas": {},
        "rollback": "python controlar_pre_live_preciso.py --suspender",
    }
    return {
        **_gravar(caminho, documento),
        "alterado": True,
...
def aplicar_validacao(validacao, *, caminho=None, agora=None):
    """Aplica apenas os dois gatilhos pre-registrados da coorte."""
    validacao = validacao or {}
    estado = ler_estado(caminho)
    validacao_inconsistente = bool(
        validacao.get("versao") and validacao.get("saudavel") is False
    )
    evidencia_negativa = bool(validacao.get("alerta_desfavoravel"))
    coorte_concluida = bool(validacao.get("resultados_completos"))
    if (
        validacao_inconsistente or evidencia_negativa or coorte_concluida
    ) and estado.get("ativo"):
        assinatura = "|".join((
        ...
        motivo = (
            ...
            if validacao_inconsistente
            else
            "ic95_roi_checkpoint_25_integralmente_negativo"
            if evidencia_negativa
            else "coorte_60_concluida_aguardando_revisao_manual"
        )
        estado = suspender(
            assinatura,
            motivo,
